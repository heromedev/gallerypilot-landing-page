  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-background">
      ...
      <main id="main">
        ...
        <section id="pricing" className="scroll-mt-20 border-y border-border bg-card px-5 py-24 lg:px-8 lg:py-32"><div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-center"><Reveal><p className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-primary">{t.pricingSection.label}</p><h2 className="mt-4 text-4xl font-semibold tracking-[-0.04em] sm:text-5xl whitespace-pre-line">{t.pricingSection.heading}</h2><p className="mt-5 max-w-md text-lg leading-relaxed text-muted-foreground">{t.pricingSection.description}</p></Reveal><Reveal delay={0.08}><article className="border border-primary/40 bg-background p-8 shadow-2xl shadow-primary/10 transition-shadow duration-500 hover:shadow-primary/20 sm:p-10"><div className="flex items-start justify-between gap-6"><div><p className="font-mono text-xs uppercase tracking-widest text-primary">{t.pricingSection.tierName}</p><p className="mt-3 text-5xl font-semibold tracking-tight">{t.pricingSection.price}</p></div><span className="rounded-full border border-border px-3 py-1 text-xs text-muted-foreground">{t.pricingSection.frequency}</span></div><ul className="mt-9 grid gap-4 sm:grid-cols-2">{t.pricingSection.features.map(x => <li key={x} className="flex gap-3"><Check className="mt-0.5 size-5 text-primary" aria-hidden="true" />{x}</li>)}</ul><PricingButton label={t.pricingSection.cta} /><p className="mt-4 text-center text-xs text-muted-foreground">{t.pricingSection.disclaimer}</p></article></Reveal></div></section>
